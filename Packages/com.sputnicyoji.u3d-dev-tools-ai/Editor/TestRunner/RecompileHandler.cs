...
namespace Yoji.TestRunner
{
    ...
    /// 有意复制自 com.sputnicyoji.u3d-dev-tools-ai 的 RecompileHandler 并改动：(1) 增加 AssetDatabase.Refresh；
    /// (2) 响应为 flat {success,message,compilationTime,hasErrors}（无 ok/elapsedMs 信封）；
    /// (3) 接 JobStore 状态机，非 Idle 返 409。未来抽 com.sputnicyoji.u3d-dev-tools-ai 时需先统一响应契约。
