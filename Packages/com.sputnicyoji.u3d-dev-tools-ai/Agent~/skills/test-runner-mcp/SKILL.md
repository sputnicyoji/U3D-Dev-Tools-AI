...
`com.sputnicyoji.u3d-dev-tools-ai` 是 Unity Editor 内的轻量 HTTP 服务，让 AI 不开 GUI 就能：触发重编译 → 等域重载 → 发起测试 → 轮询拿结果。`tdd-workflow` 在「执行测试」阶段调用的就是本接口。

> [!IMPORTANT]
> 本工具已具备公开的 Unity 侧实现（UPM 包 `com.sputnicyoji.u3d-dev-tools-ai`，EditMode +
...
```json
"com.sputnicyoji.u3d-dev-tools-ai": "file:<path-to-repo>/Packages/com.sputnicyoji.u3d-dev-tools-ai"
```

或把 `Packages/com.sputnicyoji.u3d-dev-tools-ai/` 拷进目标工程 `Packages/`。
