...
| `com.yoji.test-runner` | HTTP MCP service for Unity recompilation and EditMode/PlayMode test runs | `client.py`, `SKILL.md`, e2e script | Yes | Stable `0.1.5` target tag; Unity 2022 Git UPM smoke verified; Unity 6000 typed path retained |
| `com.yoji.editor-debug` | HTTP+JSON reflection/debug service inside Unity Editor | `client.py`, `SKILL.md`, references | Yes | Stable `0.1.1` target tag |
| `com.yoji.lua-device-debug` | HTTP+JSON diagnostics transport for project-provided Lua runtime adapters | `client.py`, `SKILL.md` | Yes | Stable `0.1.1`; Unity 2022.3 compatible; target project must register a host |
| `com.yoji.u3d-ai-linker` | Project Settings package for installing tools and syncing Claude/Codex skills/rules | Tool fragments | Yes | Stable `0.1.5` target tag includes service port hardening registry |
...
Port: legacy `21890/21896/21897`, project offset `+0`
...
Port: legacy `21891/21892/21893`, project offset `+1`
...
Port: legacy `21894`, project offset `+4`
...
| Tool | Port | Protocol |
|------|------|----------|
| `test-runner-mcp` | legacy `21890/21896/21897`, project offset `+0` | HTTP |
| `unity-editor-debug-mcp` | legacy `21891/21892/21893`, project offset `+1` | HTTP+JSON |
| `unity-lua-device-debug` | legacy `21894`, project offset `+4` | HTTP+JSON |
...
Additional `test-runner` compatibility checks:
...
2026-06-24 service-port hardening local gates passed for JSON parsing,
Python syntax, resolver behavior, and manual editor-core / lua compile.
Unity EditMode batchmode is currently blocked before script compilation by
Package Manager resolving with `The "path" argument must be of type string.
Received undefined`.
