...
>
> 最后更新: 2026-06-22 (stable registry 已更新到 test-runner-v0.1.4 / lua-device-debug-v0.1.0 / u3d-ai-linker-v0.1.4；Unity 2022.3.62f2c1 活 Editor smoke 通过；Unity 6000 typed TestRunnerApi 路径保留)
...
|------|------|------|----------|----------|------|
| test-runner-mcp | 21890 (回退 21896/21897) | HTTP | 已迁入包 | 服务包 (`com.yoji.test-runner`) | 可用 (EditMode + PlayMode via DisableDomainReload, TR-1a)；stable 0.1.4；6000.3.16f1 与 2022.3.62f2c1 验证 |
| unity-editor-debug-mcp | 21891 (回退 21892/21893) | HTTP+JSON | 已迁入包 | 服务包 (`com.yoji.editor-debug`) | 可用；6000.3.16f1 验证 |
| unity-lua-device-debug | 21894 | HTTP+JSON | 已迁入包 | 服务包 (`com.yoji.lua-device-debug`) | stable 0.1.0；Unity 2022.3+；传输层就绪；目标工程必须注册 `ILuaDeviceDebugHost`；已在 SLG_Prototype 真实 xLua 适配器端到端验证 |
| **u3d-ai-linker** | - | - (Editor 编排) | 同步 Skills/规则 | 编排包 (`com.yoji.u3d-ai-linker`) | **stable 0.1.4 已发布；stable registry 指向 editor-core-v0.1.0 / editor-debug-v0.1.0 / test-runner-v0.1.4 / lua-device-debug-v0.1.0 / u3d-ai-linker-v0.1.4；HD2D/PurgeDemo Unity 2022 Git UPM smoke 与 SLG_Prototype Agent Sync/Junction 验证通过** |
...
- TR-1a PlayMode (已落地，2026-06-15): `testMode:"PlayMode"` 经临时叠加
  ...
  editor-debug 全程在线）。
- Unity 2022 兼容修复 (2026-06-17，stable `test-runner-v0.1.4`):
...
### 4. u3d-ai-linker (stable 0.1.3，已提交+推送+打 tag)
...
- 资产模式: stable ready 范围为 `editor-core` / `editor-debug` / `test-runner` /
  `lua-device-debug` / `u3d-ai-linker`。当前 stable tag URL 为 `editor-core-v0.1.0`、
  `editor-debug-v0.1.0`、`test-runner-v0.1.4`、`lua-device-debug-v0.1.0`、
  `u3d-ai-linker-v0.1.4`；消费者 lock 应解析为 `source=git`。
...
- Stable registry 热修复 (2026-06-17):
  ...
  - `u3d-ai-linker-v0.1.3` / `test-runner-v0.1.4` 修复 Unity 2022 生成代理无法访问 private nested proxy 构造的问题。
  - HD2D-Demo 已通过 Git stable 通道安装 `test-runner-v0.1.4` 与 `u3d-ai-linker-v0.1.3`，包 hash 为
...
- Stable registry lua 发布 (2026-06-22):
  ...
  - `u3d-ai-linker-v0.1.4` 更新包内 stable/dev registry snapshot，确保 Project Settings 面板能看到 lua stable 项。
  - PurgeDemo Unity `2022.3.62f2c1` 活 Editor smoke 通过：`EditorDebugMCP`、`TestRunnerMCP`、
...
- **发布证明 (2026-06-17):**
  - `tools/check-u3d-linker-stable.ps1` 与 `tools/check-u3d-linker-stable.ps1 -RequireRemoteTags`
    均通过。
  - 已推送 `editor-core-v0.1.0`、`editor-debug-v0.1.0`、`test-runner-v0.1.4`、
    `u3d-ai-linker-v0.1.3`。
- 实现细节与权威类型收敛见
...
| unity-lua-device-debug | Unity 2022.3.62f2c1 (PurgeDemo local file smoke) | **通过**：`/ping` 正常，EditMode `14/14` 通过 |
...
- 解析结果: `com.yoji.test-runner@0.1.4` 指向 `test-runner-v0.1.4`；
...
- [x] u3d-ai-linker: stable registry 热修复已发布到 `u3d-ai-linker-v0.1.3`，内嵌 snapshot 指向
      `test-runner-v0.1.4`，根目录与包内 `stable.json` 一致。
...
- [x] test-runner-mcp: Unity 2022 兼容热修复已发布到 `test-runner-v0.1.4`；反射 adapter + public `DispatchProxy`
...
- [x] unity-lua-device-debug: 兼容 Unity 2022.3.62f2c1 并纳入 stable registry (`lua-device-debug-v0.1.0`)，`u3d-ai-linker-v0.1.4` 内嵌 registry snapshot 已同步。
