...
>
> 最后更新: 2026-06-17 (project-aware ports 已实现：多 Unity 工程不再共享固定端口；agent client 已支持 --project/--pid/--port 解析；live 双工程 smoke 待跑)
...
|------|------|------|----------|----------|------|
| test-runner-mcp | legacy 21890 (回退 21896/21897)，project offset base+0 | HTTP | 已迁入包；client 支持 --project/--pid/--port | 服务包 (`com.yoji.test-runner`) | project-aware 端口已实现；live 双工程 smoke 待跑 |
| unity-editor-debug-mcp | legacy 21891 (回退 21892/21893)，project offset base+1 | HTTP+JSON | 已迁入包；client 支持 --project/--pid/--port | 服务包 (`com.yoji.editor-debug`) | project-aware 端口已实现；live 双工程 smoke 待跑 |
| unity-lua-device-debug | legacy 21894，Editor project offset base+4；Android player 默认 21894 可配置 | HTTP+JSON | 已迁入包；client 支持 --project/--pid/--port | 服务包 (`com.yoji.lua-device-debug`) | Editor project-aware 端口已实现；Android Development Build 端口可用 RuntimeConfig 覆盖 |
| **u3d-ai-linker** | - | - (Editor 编排) | 同步 Skills/规则 | 编排包 (`com.yoji.u3d-ai-linker`) | **stable 0.1.0 已发布；UI 状态深查后 264 EditMode 绿；SLG_Prototype 四个 stable 包 tag URL 安装、Agent Sync、Repair Links、Windows Junction smoke 已活 Editor 验证；lua-device-debug 暂留 Dev/本地范围** |
...
> 端口模型已升级为 project-aware: 首个工程优先占用 legacy 端口，第二个及后续工程按稳定 project id 计算 base range。
> Agent client 解析顺序为显式 `--port` -> `--project`/cwd 的 `.u3d-ai-linker/ports.json` -> 机器 registry -> legacy 默认端口。
> 同一项目多个在线 Editor 实例会要求 `--pid` 或 `--port` 消歧。
...
- Review 修复 (2026-06-17): Local 通道读取 dev snapshot 并生成 `file:` 依赖；Dev 通道必须
  ...
  `Rollback Manifest`。Stable snapshot 已发布 ready 范围仍限定为 editor-core/editor-debug/test-runner/linker；
  lua-device-debug 暂留 planned，避免 stable 通道误装未完成的通用 host 契约。
- **发布证明 (2026-06-17):**
...
- [x] project-aware ports: test-runner/editor-debug/lua-device-debug 均已接入 editor-core allocator、项目端口文件、机器 registry、agent resolver；live 双工程 smoke 待环境可用后执行。
