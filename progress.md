...
>
> 最后更新: 2026-06-17 (u3d-ai-linker Agent Sync/Junction Repair 已实装并在 SLG_Prototype 活 Editor 验证；stable 0.1.0 范围为 editor-core/editor-debug/test-runner/u3d-ai-linker，四包 Git SHA 安装验证通过，batchmode EditMode 更新为 260/260；lua-device-debug 仍保留本地/Dev 范围)
...
| **u3d-ai-linker** | - | - (Editor 编排) | 同步 Skills/规则 | 编排包 (`com.yoji.u3d-ai-linker`) | **stable 0.1.0 RC 就绪 (260 EditMode 绿)；SLG_Prototype 四个 stable 包 Git SHA 安装、Agent Sync、Repair Links、Windows Junction smoke 已活 Editor 验证；lua-device-debug 暂留 Dev/本地范围** |
...
### 4. u3d-ai-linker (stable 0.1.0 RC，已提交+推送)
...
- 资产模式: SLG_Prototype 活 Editor 实测四个 stable 包
  (`editor-core`/`editor-debug`/`test-runner`/`u3d-ai-linker`) 从 Git SHA
  `a2a0201413bcb40719b30a809be8078441e9f691` 解析到 `Library/PackageCache`；
...
- **剩余发布动作:**
  - 创建并推送 `editor-core-v0.1.0`、`editor-debug-v0.1.0`、`test-runner-v0.1.0`、
    `u3d-ai-linker-v0.1.0` tag 后，用 `tools/check-u3d-linker-stable.ps1 -RequireRemoteTags`
    做最终远端 tag gate。
