...
>
> 最后更新: 2026-06-15 (**已提交并推送 `5a0f495`**，origin/main 已验同步；在 2c3ba9a 之上：HTTP body 硬化 + `/eval` 默认关闭 + linker 门控；**ARCH-1a** 抽 `com.yoji.editor-core`；**TR-1a** PlayMode via DisableDomainReload；三服务失焦即时恢复 (afterAssemblyReload) + `EditorServiceLifecycle` 生命周期接线收口；PlayMode agent 契约与 linker junction 回滚修正；本轮 batchmode: editor-debug 82/82、test-runner 29 total、lua-device-debug 13/13、u3d-ai-linker 246/246)
