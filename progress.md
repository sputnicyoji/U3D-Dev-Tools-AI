...
>
> 最后更新: 2026-06-15 (在 2c3ba9a 之上：HTTP body 硬化 + `/eval` 默认关闭 + linker 门控；**ARCH-1a** 抽 `com.yoji.editor-core`；**TR-1a** PlayMode via DisableDomainReload；三服务失焦即时恢复 (afterAssemblyReload)；PlayMode agent 契约与 linker junction 回滚修正；本轮 batchmode: editor-debug 82/82、test-runner 29 total、lua-device-debug 13/13、u3d-ai-linker 246/246)
...
| **u3d-ai-linker** | - | - (Editor 编排) | 同步 Skills/规则 | 编排包 (`com.yoji.u3d-ai-linker`) | **实现完成 (8 子系统 / 246 EditMode 绿)；已提交+推送 (2c3ba9a)；面板/安装/Junction 等真实副作用待 live-editor 手验** |
| com.yoji.editor-core | - | - (共享基础设施) | - | Editor-only 包 (`com.yoji.editor-core`) | 新增 (ARCH-1a)：`MainThreadDispatcher` + `RequestBodyReader` 去重宿主；editor-debug/test-runner 依赖之。活 Editor 编译加载验证 |
...
- P1 do-next: TR-3 0 命中守卫 + `/list-tests` (已落地)、ED-2 `/batch` (已落地)、
  **TR-1a PlayMode via DisableDomainReload (已落地，2026-06-15)**、**ARCH-1a 抽
  ...
  2026-06-15)**；ARCH-1b 抽 `LoopbackHttpHost`/`RecompilePrimitive` 待做。
