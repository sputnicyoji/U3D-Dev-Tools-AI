...
>
> 最后更新: 2026-06-17 (追加 SLG_Prototype 活 Editor + Android Development Build 验证；u3d-ai-linker Project Settings 的 Install/Update、Restore、Rollback 入口已接线，batchmode EditMode 更新为 247/247；lua-device-debug Android 真机 `hostReady=true` 且 5 个只读命令全 200)
...
| **u3d-ai-linker** | - | - (Editor 编排) | 同步 Skills/规则 | 编排包 (`com.yoji.u3d-ai-linker`) | **实现完成 (8 子系统 / 247 EditMode 绿)；SLG_Prototype 本地 file: 面板入口 + 探针已验；Install/Update、Restore、Rollback 入口已接线，Agent Sync/Junction Repair 仍待实装/手验** |
...
- **本次实现 (mission-runner，2026-06-14)：8 个子系统 / 50 Task 中 48 个 headless 落地；
  当前 batchmode EditMode `total=247 passed=247 failed=0`（2026-06-17，含 Project Settings Install/Update 动作服务补测）：**
...
- 资产模式: SLG_Prototype 本地 `file:` 安装实测 `Agent~/SKILL.md` 与
  `BundledSkills~` 均可读，`Tools > U3D AI Linker > Run Agent Asset Probe` 写出
  `Library/U3DAILinker/probe-result.json`（2026-06-16T15:43:58Z，3/3 readable，
  `recommendedMode=directory`）。真实 Git Package Cache 探针仍需单独空工程验证。
- Project Settings 动作接线 (2026-06-17): `Refresh Registry` 读取包内快照；
  ...
  接入现有 manifest 事务/回滚。`Sync Agent Assets` 与 `Repair Links` 仍保持禁用，
  等 resolved package path 与 Junction 副作用手验后再开放。
- **未完成 (人工域，交用户)：**
  - 真实副作用 (真实安装 / 域重载续跑 / Windows Junction / 面板渲染 / Local 通道 /
    manifest 事务对真实 manifest) 仍需 e2e/手动验证，矩阵标 `live-editor-slg`，可经
...
| u3d-ai-linker | Unity 6000.3.16f1 (batchmode) | **247/247 EditMode 通过 (纯逻辑全套；Install/Update 动作服务已测；真实副作用为 live-editor 手动)** |
...
- [ ] u3d-ai-linker: 在真实 Editor (SLG_Prototype 本地 file: 路径) 已验证 Project Settings
      入口与 Agent asset probe；Install/Update、Restore、Rollback 已接线并有 EditMode 证明，
